...
{
    // copy the content of word and convert in lower case
    int l = strlen(word);
    char *copy = malloc(l);
    ...
    //  hash the word and check the table for the node at that index
    ...
    if (entry)
    {
        ...
        // if the node isn't nule, loop through the nodes while a next pointer
...
{
    // open the input dictionary file and check for an unsuccesful open
